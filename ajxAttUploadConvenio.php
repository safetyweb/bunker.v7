...
$cod_conveni = fnLimpaCampoZero($_POST['COD_CONVENI']);
...
$sql = "SELECT * FROM ANEXO_CONVENIO WHERE $tp_anexo = $cod_busca $completaCont ORDER BY DAT_CADASTR DESC LIMIT $ini,$limit";
					...
					?>
						...
						<tr>
							...
							<?php if($file_ext == "jpeg" || $file_ext == "jpg" || $file_ext == "png"){ ?>
								<td><a href="https://adm.bunker.mk/media/clientes/<?php echo $cod_empresa; ?>/convenios/convenio.<?php echo $cod_conveni; ?>/<?php echo $qrAnexo['NOM_ORIGEM']; ?>" class="download" target="files" onclick="openNav()"><span class="fas fa-eye"></span>
								</a></td>
							<?php }else{ ?>
								<td><a href="https://docs.google.com/a/192.99.240.249/viewer?url=http://adm.bunker.mk/media/clientes/<?php echo $cod_empresa; ?>/convenios/convenio.<?php echo $cod_conveni; ?>/<?php echo $qrAnexo['NOM_ORIGEM']; ?>&pid=explorer&efh=false&a=v&chrome=false&embedded=true" class="download" target="files" onclick="openNav()"><span class="fas fa-eye"></span></a></td>
							<?php } ?>
							<td><a class="download" href="../media/clientes/<?php echo $cod_empresa; ?>/convenios/convenio.<?php echo $cod_conveni; ?>/<?php echo $qrAnexo['NOM_ORIGEM']; ?>" download><span class="fa fa-download"></span></a></td>

							<td width="70%"><?=$qrAnexo['NOM_REFEREN']?></td>
							<td width="20%">
								<div class="row">
									<div class="col-md-6">
										<?=$status?>
...
function atualizaAnexo(id_doc){
	...
	$("#fileinput").fileinput("destroy").fileinput({
			...
			uploadUrl: "ajxSubDocConvenio.php?idd="+id_doc+"&tipo=<?=$tp_anexo?>&ido="+$("#COD_OBJETOANEXO").val()+"&tpc=<?=fnEncode($tp_cont)?>&cod_empresa=<?=$cod_empresa?>&cod_conveni=<?=$cod_conveni?>&num_contador=<?=$num_contador?>",
			uploadAsync: true,
