...
// Configurar valores padrão para índices esperados em $_POST, $_GET, $_SESSION, etc.
$default_values = [
    'COD_STATUS' => '0',        // Status padrão
    'SYS_COD_EMPRESA' => '7', // Código da empresa padrão
    'OUTRA_CONSTANTE' => 'valor_padrao', // Outro valor padrão
];

// Função para definir valores padrão em arrays
function applyDefaultValues(array &$target, array $defaults)
{
...
// Aplicar valores padrão globalmente
applyDefaultValues($_POST, $default_values);
applyDefaultValues($_GET, $default_values);
applyDefaultValues($_SESSION, $default_values);
...
{
    // Remove palavras-chave SQL e barras invertidas
...
{
    // Remove palavras-chave SQL, caracteres especiais e barras invertidas
    $campo = preg_replace("/(from|alter table|select|drop|insert|delete|update|where|drop table|show tables|#|\*|--|\\\\)/i", "", $campo);
...
function fnValor($Num, $Dec)
{
    ...
    if ($Dec == '0') {
        // Multiplicação e divisão sem funções de precisão arbitrária
        $valor = ($Numero * 100) / 100;
        $valor = number_format($valor, $Dec, ",", ".");
    } else {
        // Multiplicação e divisão com arredondamento para garantir precisão
        $valor = round(($Numero * 100) / 100, $Dec);
        $valor = number_format($valor, $Dec, ",", ".");
    }
...
function fnScan($arquivo)
{
    ...
    if ($socket && socket_connect($socket, '/var/run/clamav/clamd.ctl')) {
        ...
        if (trim($PING) == 'PONG') {
            chmod($arquivo['CAMINHO_TMP'], 0644);
            $socket = socket_create(AF_UNIX, SOCK_STREAM, 0);
            if ($socket && socket_connect($socket, '/var/run/clamav/clamd.ctl')) {
                $result = "";
                $comando = "SCAN " . $arquivo['CAMINHO_TMP'];
                socket_send($socket, $comando, strlen($comando), 0);
                socket_recv($socket, $result, 20000, 0);
                socket_close($socket);

                $quebradelina = explode(':', $result);
                if (isset($quebradelina[1]) && trim($quebradelina[1]) == 'OK') {
                    return array(
                        'RESULTADO' => 0,
                        'MSG' => 'N'
                    );
                    ...
                    return array(
                        'RESULTADO' => 1,
                        'MSG' => isset($quebradelina[1]) ? trim($quebradelina[1]) : "Erro desconhecido"
                    );
                }
            }
        }
...
{
    // Se a mensagem estiver vazia (null, string vazia, etc.), retorna imediatamente para seguir o fluxo
    ...
    // Se a decodificação for bem-sucedida, retorna o valor decodificado; caso contrário, retorna o original
