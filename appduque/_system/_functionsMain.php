...
ini_set('session.gc_maxlifetime', 1800000); // 1 hora
...
function carregaPagina($vf){
    ...
    elseif ($vf='false') {
     ...
     }else{ echo "Você esqueceu de carregar a variavel com True para inicio e false para final";}
}
...
{
  if ( !is_numeric($Num) || empty($Num) || is_null($Num) ) {$Numero = 0;} else {$Numero = $Num;}		
...
elseif($opcao='false'){
   ...
   // $finaltime1=(microtime(TRUE) - $time);
    ...
    $tempo_carregamento = microtime(TRUE); - $_SERVER['REQUEST_TIME'];
         if ($mem_usage < 1024)
        {    
...
function get_name_sistem($user_agent)
{
