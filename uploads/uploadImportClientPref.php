				...
				*/
				...
				if($inseriu){
					...
					mysqli_query(connTemp($cod_empresa,""),"UPDATE import_clientes
															SET LOG_IMPORT='N', MSG_ERRO='Telefone não preenchido!'
																WHERE TRIM(IFNULL(TEL_1,'')) = '' 
...
?>
			...
			<script>
				...
				$.ajax({
					type: "GET",
					url: "../uploads/uploadImportClientPref.php?acao=confirmar&id=<?php echo $cod_empresa; ?>",
				...
				$('#loadMore').click(function(){
					...
					if(cont >= "<?php echo $qrLinhas['LINHAS']; ?>"){
						...
						$('#loadMore').text('Todos os Itens Já se Encontram na Lista');
					}

					$.ajax({
						type: "GET",
						url: "../uploads/uploadImportClientPref.php?acao=loadMore&itens="+cont+"&id=<?php echo $cod_empresa; ?>",
						beforeSend:function(){	
		...
		<script>
			...
			$('.next3').click(function(){
					$.ajax({
						type: "POST",
						url: "../uploads/uploadImportClientPref.php?id=<?php echo $cod_empresa; ?>",
...
<?php 
		...
		default:
			...
			if(isset($_POST['RADIO'])){
				...
				<div class="col-md-2">
					<a href="action.do?mod=<?php echo fnEncode(1035)."&id=".fnEncode($cod_empresa)?>" class="col-md-12 btn btn-success concluir" target="_parent">Concluir</a>
				</div>
