...
if ($siteGo == "OK"){
	...
	if($qrBuscaDadosEmpresa['TIP_RETORNO'] == 1){
		$casasDec = 0;
		$pref = "Pontos: ";
	}else{
		$casasDec = $qrBuscaDadosEmpresa['NUM_DECIMAIS_B'];
		$pref = "R$ ";
	}
	...
	.p-top10{
						   ...
						   <!-- Logo goes here - replace the image with yours -->
							...
							<div class="collapse navbar-collapse main-nav" id="navigation">
								...
								<ul class="nav pull-right">
									<!-- Menu -->
									<li class='active'><a href='https://<?=$des_dominio.$extensaoDominio?>'>Home</a></li>
									<?php
				...
				<form data-toggle="validator" role="form2" method="post" id="formulario" autocomplete="off">
					...
					<!-- </div> -->
						...
						<div class="col-md-10 col-md-offset-1">
							...
							<div class="col-xs-12 gray-round">
								...
								<div class="col-xs-6 text-left">
									...
									<p class="p-top10" style="color:#fff; font-size:12px;">Prazo para utilizar os créditos:</p>

								</div>

								<div class="col-xs-6 text-right">
									
									<p class="p-top10" style="color:#fff; font-size:11px;"><b style="font-size:16px;"><?=$dat_minima?></b> <!-- A <b style="font-size:16px;"><?=$dat_minima?></b> --></p>
							...
							<div class="col-xs-12 gray-round">
								...
								<div class="col-xs-6 text-left">
									...
									<p class="p-top10" style="color:#fff; font-size:12px;">Regras:</p>

								</div>

								<div class="col-xs-6 text-right">
									
									<p class="p-top10" style="color:#fff; font-size:11px;padding-top: 10px; padding-bottom: 10px;"><a  class="btn btn-xs btn-info" id="btnExtrato" style="margin-top: 0px;">Ver Extrato</a></p>
						...
						?>	
							...
							<!-- <div class="col-md-10 col-md-offset-1">
								<a href="ticket_V2.do?key=<?php echo $_GET['key']; ?>&url=<?php echo $urlTKT['url']; ?>&ch=3" name="TKT" id="TKT" <?=$disable?> class="btn btn-info btn-lg btn-block <?php echo $tktOff; ?> <?=$disable?>" tabindex="5"><i class="fa fa-ticket" aria-hidden="true"></i>&nbsp; Visualizar Minhas Ofertas</a>
							</div>
						...
						if ($log_nps == 'S'){
							...
							$sqlPlayer = "SELECT COD_PLAYERS FROM TOTEM_PLAYERS 
										  WHERE COD_EMPRESA = $cod_empresa
										  AND DES_PAGHOME = 'cad'
										  LIMIT 1";

							$arrayPlayer = mysqli_query(connTemp($cod_empresa, ''), $sqlPlayer);
							$qrPlayer = mysqli_fetch_assoc($arrayPlayer);

							if(mysqli_num_rows($arrayPlayer) > 0){

						?>	
							
							<div class="col-md-12 col-xs-12">
							
								<iframe frameborder="0" id="blocoPesquisa" src="https://<?=$des_dominio.$extensaoDominio?>/pesquisa?idP=<?=fnEncode($cod_pesquisa)?>&idc=<?=fnEncode($cod_cliente)?>&cod_players=<?=fnEncode($qrPlayer[COD_PLAYERS])?>&hs=1" style="width: 100%; height: 70vh;" sandbox="allow-top-navigation allow-scripts allow-forms allow-popups allow-popups-to-escape-sandbox"></iframe>
		...
		<script>
			...
			parent.$('html, body').animate({
		        scrollTop: parent.$("#extrato").offset().top + 120
		    }, 1000);
