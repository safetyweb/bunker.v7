...
switch ($opcao) {
	...
	default:
		...
		if($cod_persona != 0){
			...
			// fnEscreve($sqlUpdt);
			mysqli_query(connTemp($cod_empresa,''),$sqlUpdt) or die(mysqli_error());
		}
		...
		{		
			...
			if ($log_ativo == "S"){$personaaAtivo = "<i class='fal fa-check' aria-hidden='true'></i>";}
			else {$personaaAtivo = "";}
			
			if ($qrListaPersonas['LOG_RESTRITO'] == "S"){$personaaAtualiza = "<i class='fal fa-check' aria-hidden='true'></i>";}
