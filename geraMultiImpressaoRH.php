<?php
	...
	// fnEscreve($cod_modulo);
	
	//tipo de lançamento	
	switch ($cod_modulo) {
	case 1743: //folha de pagamento avulso
		$tip_lancame = "F";
		$andBonifica = " ";
		$andContabiliza = " ";
		$andTipo = " AND TIP_CREDITO.COD_TIPO = '$cod_tipo'";
		break;
	case 1739: //folha de pagamento geral
		$tip_lancame = "F";
		$andBonifica = "";
		$andContabiliza = " ";
		$andTipo = " ";
		break;
	case 1744: //bonificação avulsa
		$tip_lancame = "B";
		$andContabiliza = " ";
		$andBonifica = "AND CL.LOG_BONIFICA = 'S'";
		$andTipo = " AND TIP_CREDITO.COD_TIPO = '$cod_tipo'";
		break;
	case 1738: //bonificação geral
		$tip_lancame = "B";
...
<?php
	...
	if ($cod_modulo == 1743 || $cod_modulo == 1744){
	//impressão avulsa	
		$sqlCli = "SELECT CL.COD_CLIENTE, CL.NOM_CLIENTE
					FROM CLIENTES CL,CAIXA B
					WHERE 
					CL.COD_CLIENTE=B.COD_CONTRAT AND
					B.COD_MES = $cod_mes AND  
					B.COD_TIPO = $cod_tipo AND
					B.TIP_LANCAME = '$tip_lancame' AND
					CL.COD_EMPRESA = $cod_empresa AND 
					CL.LOG_TITULAR = 'S' AND 
					CL.LOG_ESTATUS = 'S'
					$andBonifica
					";
	}else{
	...
	while($qrCli = mysqli_fetch_assoc($arrayCli)){
		...
		FROM CAIXA
		inner join TIP_CREDITO on caixa.COD_TIPO=TIP_CREDITO.COD_TIPO $andContabiliza
	...
	<div class="row" style="height: 535px;" id="primeiraImpressao">

		<div class="col-xs-10" style="height: 535px;">
		...
		<div class="col-xs-2 text-center" style="height: 533px; margin-left: -15px; border: 1px solid #ecf0f1;">
			<p class="f10 verticaltext" style="position: absolute; width: 533px; height: 100px; top: 220px; left: -200px;">
	...
	<div class="row" style="height: 535px;" id="segundaImpressao" style="display: none; margin-bottom: -50px;">
