...
<?php 
                ...
                // echo "$gravatokem";
                // exit();
                mysqli_query(connTemp($cod_empresa,''), $gravatokem); 
...
<?php 
        ...
        $sql2="SELECT COD_CLIENTE, NUM_CGCECPF FROM clientes WHERE NUM_CARTAO=$usuario and COD_EMPRESA=$cod_empresa";
...
<style>
            ...
            <div class="row">
                <div class="col-xs-10 col-xs-offset-1 text-center">
                    ...
                    ?>
                            <div class="text-left">
                                <div class="push20"></div>
                                <label><b>Unidade de Preferência</b></label>
                                <div class="form-group">
                                    <select data-placeholder="Selecione um estado" name="COD_UNIVENDPREF" id="COD_UNIVENDPREF" class="chosen-select-deselect input-sm">
                                        <?php
                                            ...
                                            $sqlUniv = "SELECT * FROM (
                                                            ...
                                                            WHERE v.cod_cliente = $cod_cliente_consulta AND v.cod_empresa = 19 
                                                            ORDER BY v.cod_venda DESC LIMIT 1
                                                        )AS ranked
                                                        UNION ALL
                                                        SELECT * FROM (
                                                        SELECT unid.COD_UNIVEND,unid.NOM_FANTASI FROM unidadevenda unid WHERE cod_empresa=19 AND unid.LOG_ESTATUS='S' ORDER BY unid.NOM_FANTASI ASC 
                                                        )ranked2";
                                            ...
                                            while($qrUniv = mysqli_fetch_assoc($arrUniv)){
                                                ...
                                                if($countUniv == 0){
                                                    $countUniv++;
                                                    $cod_univendpref = fnEncode($qrUniv['COD_UNIVEND']);
                                                }

                                        ?>
                                                <option value="<?=fnEncode($qrUniv['COD_UNIVEND'])?>"><?=$qrUniv['NOM_FANTASI']?></option>
                    ...
                    ?>
                            <div class="push30"></div>
                            <a href="javascript:void(0)" onclick='submitForm("resg")' id="btnGeraTokenResgate" class='btn btn-block' style="background-color: #1E8449; color: #fff;"><i class="fal fa-money-bill" aria-hidden="true"></i>&nbsp;&nbsp;Gerar Token de <b>Resgate</b></a>
                    <?php 
                        } 
                    ?>
                            <div class="push20"></div>
                            <div class="push30"></div>
                            <!-- <div class="push10"></div> -->
                            <a href="javascript:void(0)" onclick='submitForm("desc")' id="btnGeraToken" class='btn btn-primary btn-block'><i class="fal fa-unlock-alt" aria-hidden="true"></i>&nbsp;&nbsp;Gerar Token de <b>Desconto</b></a>
    ...
    <form id="formulario" type="GET" action="validaDadosToken.php">
        <input type="hidden" id="key" name="key" value="<?=$_GET['key']?>">
        <input type="hidden" id="t" name="t" value="<?=$rand?>">
        <input type="hidden" id="idU" name="idU" value="<?=$_GET['idU']?>">
        <input type="hidden" id="idp" name="idp" value="<?=$des_placa?>">
        <input type="hidden" id="uni" name="uni" value="<?=$cod_univendpref?>">
        <input type="hidden" id="tp" name="tp" value="">
        <input type="hidden" id="validaToken" name="validaToken" value="N">
    </form>
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            $('input[type=radio][name=DES_PLACA]').change(function() {
                let des_placa = $(this).val();
                $("#idp").val(des_placa);
            });

            $('#COD_UNIVENDPREF').change(function() {
                let univendPref = $(this).val();
                $("#uni").val(univendPref);
                $("#validaToken").val('S');
            });
        ...
        function submitForm(tpToken){
            $("#tp").val(tpToken);
            if(tpToken != "desc"){
                if($("#validaToken").val() == 'S'){
                    $("#formulario").attr("action","validaDados_v2.php");
                }
            }
            $("#formulario").submit();
        };
