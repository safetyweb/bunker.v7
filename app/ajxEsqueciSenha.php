...
switch($opcao){
	...
	case 'CPF':
		...
		if(mysqli_num_rows($arrayQuery) == 1){
			...
			if($anoNascimento == ""){
			?>
				<script>trocaSenha("CLIENTE");</script>
			<?php
			}
...
?>
            ...
            <form class="form-signin" method="post" action="reenvioSenha.do?key=<?=fnEncode($cod_empresa)?>">
...
?>
            ...
            <form data-toggle="validator" role="form2" method="post" id="formulario" action="recuperarSenha.do?key=<?=fnEncode($cod_empresa)?>">
                <label for="DES_ANO" class="sr-only">ANO</label>
