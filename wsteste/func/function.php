...
$ID_FORMPA="SELECT last_insert_id(ID) as ID from log_men ORDER by ID DESC limit 1;";
$COD_FORMAPA = mysqli_fetch_assoc(mysqli_query($conn,$ID_FORMPA));
$ID=$COD_FORMAPA['ID'];
...
function fn_consultaBase($conn,$CPF,$CNPJ,$cartao,$email,$telcelular,$empresa){
   if($CPF!="")
   {
       $sql="SELECT count(COD_CLIENTE) as contador,clientes.* FROM clientes where COD_EMPRESA=$empresa and NUM_CGCECPF=".$CPF; 
   ...
   if($CNPJ!='')
   {
       $sql="SELECT * FROM clientes where COD_EMPRESA=$empresa and NUM_CGCECPF=".$CNPJ; 
    ...
    if($cartao!='')
   {
       ...
       $sql="SELECT * FROM clientes where COD_EMPRESA=$empresa and NUM_CARTAO='".$cartao."'"; 
       $row1 =   $row1 = mysqli_fetch_assoc(mysqli_query($conn,$sql)); 
        $arraydadosBase=array();
        array_push($arraydadosBase, array(
...
{
    $sqlbusca="select count(*) as exist,COD_USUARIO,NOM_USUARIO,COD_TPUSUARIO,COD_MULTEMP,COD_UNIVEND,COD_DEFSIST from usuarios where COD_MULTEMP='$COD_MULTEMP' and NOM_USUARIO='".$NOM_USUARIO."'";
   ...
   //NOM_USUARIO, COD_TPUSUARIO = 7, COD_MULTEMP = COD_EMPRESA, COD_UNIVEND = LOJA, COD_DEFSIST = 4
    $sql='insert into usuarios (NOM_USUARIO,COD_TPUSUARIO,COD_MULTEMP,COD_UNIVEND,COD_DEFSIST)
                                values
                                (
                                "'.$NOM_USUARIO.'",
                                "7",
