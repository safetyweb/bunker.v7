<?php 
	...
	switch($opcao){
		...
		case 'exc':
			...
			$sql = "DELETE FROM TEMPLATE_AUTOMACAO_WHATSAPP WHERE COD_EMPRESA = $cod_empresa AND COD_TEMPLATE = $cod_template";
			// fnEscreve($sql);
			mysqli_query(connTemp($cod_empresa,''),$sql);

			switch ($tipo) {
				case "msg":
					$sql = "DELETE FROM MENSAGEM_WHATSAPP WHERE COD_EMPRESA = $cod_empresa AND COD_TEMPLATE_BLOCO = $cod_template";
					// fnEscreve($sql);
					mysqli_query(connTemp($cod_empresa,''),$sql);
				break;
				case "wait":
					$sql = "DELETE FROM AGUARDO_WHATSAPP WHERE COD_EMPRESA = $cod_empresa AND COD_TEMPLATE = $cod_template";
					// fnEscreve($sql);
					mysqli_query(connTemp($cod_empresa,''),$sql);
				break;
			}
		...
		default:
			...
			$sql = "SELECT * FROM TEMPLATE_AUTOMACAO_WHATSAPP 
					WHERE COD_TEMPLATE = (SELECT MAX(COD_TEMPLATE) FROM TEMPLATE_AUTOMACAO_WHATSAPP 
