...
$sql = "SELECT COUNT(0) QTD,TIMESTAMPDIFF(MINUTE,MAX(DATAHORA_INICIO),NOW()) TEMPO FROM gatilhos_logs_exec WHERE TIPO='EMAIL' AND DATAHORA_ATUALIZACAO_EMPRE IS NULL";
...
/*
		...
		fnLog(array("DESCRICAO"=>"EMAIL da blacklist e em branco removidos","COD_EMPRESA"=>$cod_empresa,"SQL"=>$sql,"ERRO"=>mysqli_error($contemporaria),"COD_GATILHO"=>$cod_gatilho,"TIP_GATILHO"=>$tip_gatilho));
...
*/
		...
		$sqlcli_cad = "SELECT DISTINCT $selectCliente
						...
						AND C.COD_CLIENTE in (SELECT COD_CLIENTE FROM email_fila WHERE                                                                                                  
											...
											".($tip_gatilho == "aniv"||$tip_gatilho == "credExp"||$tip_gatilho == "inativos"?"DATE(email_fila.DT_CADASTR)=DATE(NOW()) AND":"")."
