...
while ($rsempresa = mysqli_fetch_assoc($rwempresa)) {
    ...
    while ($rsgatilho = mysqli_fetch_assoc($rwgatilho)) {
        ...
        if ($log_processa != 'S') {
            fnLog(array("DESCRICAO" => "Campanha não habilitada para envio de SMS (LOG_PROCESSA_SMS = 'N')!", "COD_EMPRESA" => $cod_empresa, "COD_GATILHO" => $cod_gatilho, "TIP_GATILHO" => $tip_gatilho, "COD_CAMPANHA" => $cod_campanha, "LAYOUT" => "text-danger"));
            continue;
        }
        ...
        if (@$_GET["ENVIAR"] == "S") {
            ...
            if ($des_periodo == 7 || $des_periodo == 15) {
                ...
                $de = ($tip_gatilho == "aniv" ? 1 : ($tip_gatilho == "inativos" ? 2 : 1));
            ...
            switch ($tags[$i]) {
                ...
                case '<#DATAEXPIRA>';
                    $selectCliente .= "(SELECT
                                        ...
                                        WHERE DAT_EXPIRA >= NOW() AND  cod_CLIENTE=C.cod_CLIENTE AND                                                                    
                                        DATE(DAT_EXPIRA) >= DATE(DATE_ADD(NOW(),INTERVAL $dias_anteced DAY)) AND
                                        DATE(DAT_EXPIRA) <= DATE(DATE_ADD(NOW(),INTERVAL " . ($dias_anteced + $des_periodomax) . " DAY))) AS DAT_EXPIRA,";
                    break;
                case '<#DATAEXPIRAMAX>';
                    $selectCliente .= "(SELECT
                ...
                case '<#SALDOEXPIRA>';
                    $selectCliente .= "(SELECT MIN(VAL_EXPIRAR) FROM email_fila WHERE
												email_fila.COD_CLIENTE=C.COD_CLIENTE AND
                                                email_fila.TIP_GATILHO='$tip_gatilho' AND
												email_fila.TIP_FILA IN (3,7,8,10,14) AND
												email_fila.COD_EMPRESA=$cod_empresa AND
												email_fila.COD_CAMPANHA=$cod_campanha AND
        ...
        while ($rsemail_fila = mysqli_fetch_assoc($rwsql)) {
            ...
            $textoenvio = str_replace('<#NOME>', limitarTextoComunicacao(@$NOM_CLIENTE[0], 13), $textoenvio);
            $textoenvio = str_replace('<#SALDO>', @$rsemail_fila['CREDITO_DISPONIVEL'], $textoenvio);
            $textoenvio = str_replace('<#NOMELOJA>', limitarTextoComunicacao(@$rsemail_fila['NOM_FANTASI'], 20), $textoenvio);
            ...
            $textoenvio = limitarTextoComunicacao($textoenvio, 159);
            ...
            $textoenvio = fnAcentos($textoenvio);
        ...
        foreach ($CLIE_CAD_L as $k => $CLIE_CAD) {
            ...
            if ($rsempresa['COD_PARCOMU'] == 22) {
                $rsempresa['DES_AUTHKEY'] = 'basic ' . base64_encode($rsempresa['DES_USUARIO'] . ':' . $rsempresa['DES_AUTHKEY']);
            }

            try {
                $retornoSMS = fnenviosms(
                    array(
                        'PROVEDOR' => $rsempresa['COD_PARCOMU'],
                        'URL' => $rsempresa['URL_API'],
                        'METHOD' => 'POST',
                        'Authorization' => $rsempresa['DES_AUTHKEY'],
                        'Usuario' => $rsempresa['DES_USUARIO'],
                        'COD_EMPRESA' => $cod_empresa,
                        'SEND' => $CLIE_SMS,
                    )
                );
