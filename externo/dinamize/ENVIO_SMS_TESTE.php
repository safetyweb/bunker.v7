...
//https://bunker.mk/externo/dinamize/ENVIO_SMS_TWILO.php?COD_EMPRESA=328&TIP_GATILHO=inativos&ENVIAR=S
...
echo "<pre>";
fnLog(array("DESCRICAO" => "Início da Rotina TESTE", "LAYOUT" => "text-success"));
...
if ((int) $horaatual == 0 && (int) $minutoatual <= 30) {
    ...
    $rs = mysqli_query($conadmin, $sql);
    fnLog(array("DESCRICAO" => "Limpa registros da tabela gatilhos_logs com mais de 90 dias de cadastro", "SQL" => $sql, "COD_EMPRESA" => $cod_empresa, "ERRO" => mysqli_error($contemporaria)));

    $sql = "DELETE FROM gatilhos_logs_exec WHERE DATE_ADD(DATAHORA_INICIO,INTERVAL 90 DAY) < DATE(NOW())";
    $rs = mysqli_query($conadmin, $sql);
    fnLog(array("DESCRICAO" => "Limpa registros da tabela gatilhos_logs_exec com mais de 90 dias de cadastro", "SQL" => $sql, "COD_EMPRESA" => $cod_empresa, "ERRO" => mysqli_error($contemporaria)));
}
...
while ($rsempresa = mysqli_fetch_assoc($rwempresa)) {
    ...
    $sql = "SELECT COUNT(0) QTD,GROUP_CONCAT(COD_LOG_EXEC) CODS FROM gatilhos_logs_exec "
        ...
        . " AND TIMESTAMPDIFF(MINUTE,DATAHORA_INICIO,NOW()) < 300";
    $rs = mysqli_query($conadmin, $sql);
    $linha = mysqli_fetch_assoc($rs);
    ...
    while ($rsgatilho = mysqli_fetch_assoc($rwgatilho)) {
        ...
        //VERIFICA se a campanha está ativa para envio de SMS
        if ($log_processa != 'S') {
            fnLog(array("DESCRICAO" => "Campanha não habilitada para envio de SMS!", "COD_EMPRESA" => $cod_empresa, "COD_GATILHO" => $cod_gatilho, "TIP_GATILHO" => $tip_gatilho, "COD_CAMPANHA" => $cod_campanha, "LAYOUT" => "text-danger"));
            continue;
        }
*/
        //Parâmetros de envio
        ...
        if (@$_GET["ENVIAR"] == "S") {
            ...
            if ($des_periodo == 7 || $des_periodo == 15) {
                ...
                fnLog(array("DESCRICAO" => "Dia da semana: $d / Antecedência: $dias_anteced", "COD_EMPRESA" => $cod_empresa, "COD_GATILHO" => $cod_gatilho, "TIP_GATILHO" => $tip_gatilho, "COD_CAMPANHA" => $cod_campanha));
                if ($d != "01" && $d != "1") {
                    $d1 = date("d", strtotime(date("Y-m-01", strtotime("+1 month")) . " - 1 days"));
                    $d2 = date("d", strtotime(date("Y-m-01", strtotime("+1 month")) . " - 2 days"));

                    fnLog(array("DESCRICAO" => "Gatilho mensal fora do período. (Período: $des_periodo / Antecedência: $dias_anteced / Referência: DIA 1&ordm; / D-0: $d / D-1: $d1 / D-2: $d2)", "COD_EMPRESA" => $cod_empresa, "COD_GATILHO" => $cod_gatilho, "TIP_GATILHO" => $tip_gatilho, "COD_CAMPANHA" => $cod_campanha));

                    if ($tip_gatilho == "aniv" && $d == $d2) {
                        //
                        // 2 dias antes do dia primeiro
                        $dat_agendamento = date("Y-m-01", strtotime("+1 month")) . " " . $tip_momento . ":00:00";
                        $tip_momento = 18; //Muda para processar as 18h
                        ...
                        //
                    } elseif ($tip_gatilho == "inativos" && $d == $d1) {
                        //
                        // 1 dia antes do dia primeiro
                        $dat_agendamento = date("Y-m-01", strtotime("+1 month")) . " " . $tip_momento . ":00:00";
                        $tip_momento = 21; //Muda para processar as 21h
                        fnLog(array("DESCRICAO" => "Gatilho mensal habilitado para envio agendado - Agendado para: $dat_agendamento / Horário Agendamento: $tip_momento", "COD_EMPRESA" => $cod_empresa, "COD_GATILHO" => $cod_gatilho, "TIP_GATILHO" => $tip_gatilho, "COD_CAMPANHA" => $cod_campanha, "LAYOUT" => "text-warning"));
                        //
                    } else {
                        continue;
        ...
        if ($gravacao) {
            if ($tip_gatilho == "aniv" || $tip_gatilho == "anivDia" || $tip_gatilho == "anivSem" || $tip_gatilho == "anivQuinz" || $tip_gatilho == "anivMes") {
                ...
                $sqlIns = "INSERT IGNORE INTO email_fila ("
                    ...
                    . " LOG_BLACKLIST_SMS,DT_CADASTR,COD_EMPRESA,COD_UNIVEND,COD_CLIENTE,NUM_CGCECPF,NOM_CLIENTE,DT_NASCIME,DES_EMAILUS,NUM_CELULAR,VAL_RESGATE,"
        ...
        for ($i = 0; $i < count($tags); $i++) {
            ...
            switch ($tags[$i]) {
                ...
                case '<#NOMELOJA>';
                    ...
                    email_fila.TIP_FILA IN (3,7,8,10,14) AND
        ...
        $selectCliente .= "C.COD_CLIENTE,C.NUM_CELULAR,email_fila.DT_CADASTR";
        ...
        $sqlcli_cad = "SELECT DISTINCT $selectCliente
            ...
            : "INNER JOIN personaclassifica per ON per.COD_CLIENTE=email_fila.COD_CLIENTE AND  per.COD_PERSONA IN (0$cod_personas)"
						...
						AND email_fila.SCAN_OPOUT_SMS='S'
						AND email_fila.TIP_FILA IN (3,7,8,10,14)
        ...
        while ($rsemail_fila = mysqli_fetch_assoc($rwsql)) {
            ...
            if ($tip_controle == 7) {
                ...
                $linha = mysqli_fetch_assoc($rs);
                if ($linha["QTD"] > 0) {
                    $msg_env .= "<br>" . "$desc_cliente - SMS já enviado esta SEMANA!";
                    //fnLog(array("DESCRICAO"=>"$desc_cliente - SMS já enviado esta SEMANA!","COD_EMPRESA"=>$cod_empresa,"SQL"=>$sql,"ERRO"=>mysqli_error($contemporaria),"COD_GATILHO"=>$cod_gatilho,"TIP_GATILHO"=>$tip_gatilho, "COD_CAMPANHA" => $cod_campanha));
                    continue;
                }
                ...
                $linha = mysqli_fetch_assoc($rs);
                if ($linha["QTD"] > 0) {
                    $msg_env .= "<br>" . "$desc_cliente - SMS já enviado dentro de 15 DIAS!";
                    //fnLog(array("DESCRICAO"=>"$desc_cliente - SMS já enviado dentro de 15 DIAS!","COD_EMPRESA"=>$cod_empresa,"SQL"=>$sql,"ERRO"=>mysqli_error($contemporaria),"COD_GATILHO"=>$cod_gatilho,"TIP_GATILHO"=>$tip_gatilho, "COD_CAMPANHA" => $cod_campanha));
                    continue;
                }
                ...
                $linha = mysqli_fetch_assoc($rs);
                if ($linha["QTD"] > 0) {
                    $msg_env .= "<br>" . "$desc_cliente - SMS já enviado este HOJE!";
            ...
            //$tel = "551100000000";/*****************************************************************************************************************/
            $CLIE_SMS_L[$lote][] = array(
                "COD_CLIENTE" => $rsemail_fila["COD_CLIENTE"],
                "from" => $DES_CLIEXT,
                "to" => '+55' . fnLimpaDoc($tel),
                "mensagem" => fnAcentos($textoenvio),
        ...
        fnLog(array("DESCRICAO" => "<b>Qtd. Lotes Gerados:</b> " . count($CLIE_CAD_L), "COD_EMPRESA" => $cod_empresa, "COD_GATILHO" => $cod_gatilho, "TIP_GATILHO" => $tip_gatilho, "COD_CAMPANHA" => $cod_campanha, "QTD_FILA" => $qtd_total_envio));
        //print_r($CLIE_CAD_L);exit;

        /*
        echo "DORME 10 ".date("Y-m-d h:i:s");
        sleep(10);
        echo "DORMIU 10 ".date("Y-m-d h:i:s");
        fnLog(array("DESCRICAO" => "PARAR AQUI!", "COD_EMPRESA" => $cod_empresa, "COD_GATILHO" => $cod_gatilho, "TIP_GATILHO" => $tip_gatilho, "COD_CAMPANHA" => $cod_campanha, "LAYOUT" => "text-danger"));
        continue;*/

        foreach ($CLIE_CAD_L as $k => $CLIE_CAD) {

            //Checa se campanha ainda está ativa
            $sql = "SELECT gt.LOG_STATUS,cp.LOG_ATIVO FROM gatilho_sms gt"
                . " INNER JOIN campanha cp ON gt.COD_CAMPANHA=cp.COD_CAMPANHA"
            ...
            $cod_lote = $k;
            fnLog(array("DESCRICAO" => "Lote: $cod_lote", "COD_EMPRESA" => $cod_empresa, "JSON" => json_encode($CLIE_SMS), "COD_GATILHO" => $cod_gatilho, "TIP_GATILHO" => $tip_gatilho, "COD_CAMPANHA" => $cod_campanha));
            ...
            fnLog(array("DESCRICAO" => "Iniciando Disparo de SMS", "COD_EMPRESA" => $cod_empresa, "COD_GATILHO" => $cod_gatilho, "TIP_GATILHO" => $tip_gatilho, "COD_CAMPANHA" => $cod_campanha));

            $base64 = base64_encode($rsempresa['DES_USUARIO'] . ':' . $rsempresa['DES_AUTHKEY']);

            try {
                sleep(20);
                fnLog(array("DESCRICAO" => "<b>SMS FAKE DISPARADO</b>", "COD_EMPRESA" => $cod_empresa, "COD_GATILHO" => $cod_gatilho, "TIP_GATILHO" => $tip_gatilho, "COD_CAMPANHA" => $cod_campanha, "layout" => "text-success"));
                //$retornoSMS = sms_twilo($base64, $CLIE_SMS, $rsempresa['DES_USUARIO'], $rsempresa['DES_AUTHKEY']);
            } catch (Exception $e) {
                //Subtrai do total
                $qtd_total_envio = $qtd_total_envio - $qtd_lote;
                fnLog(array("DESCRICAO" => "<b>Não Enviado</b> -  Par&acirc;metros de Disparo: " . $disparo, "COD_EMPRESA" => $cod_empresa, "JSON" => json_encode($retornoSMS), "COD_GATILHO" => $cod_gatilho, "TIP_GATILHO" => $tip_gatilho, "ERRO" => $e->getMessage(), "COD_CAMPANHA" => $cod_campanha, "LAYOUT" => "text-danger"));
                continue;
            }
            ...
            $sqlControle = "INSERT INTO log_nuxux(
							...
							NOW(),
							2,
							'" . str_replace('"', '', str_replace("'", '', json_encode($retornoSMS))) . "')";
            ...
            fnLog(array("DESCRICAO" => "Disparo de SMS (EnvioSms) / Par&acirc;metros de Disparo: " . $disparo, "COD_EMPRESA" => $cod_empresa, "JSON" => json_encode($retornoSMS), "COD_GATILHO" => $cod_gatilho, "TIP_GATILHO" => $tip_gatilho, "COD_CAMPANHA" => $cod_campanha));

            fnLog(array("DESCRICAO" => "Rotina SMS processada!", "COD_EMPRESA" => $cod_empresa, "COD_GATILHO" => $cod_gatilho, "TIP_GATILHO" => $tip_gatilho, "JSON" => json_encode($retornoSMS), "COD_CAMPANHA" => $cod_campanha));

            if (count(@$retornoSMS) > 0) {
                ...
                fnLog(array("DESCRICAO" => "Vai entrar na rotina de SUCESSO! Total de envios: " . count($retornoSMS), "COD_EMPRESA" => $cod_empresa, "SQL" => $sqlControle, "ERRO" => mysqli_error($contemporaria), "COD_GATILHO" => $cod_gatilho, "TIP_GATILHO" => $tip_gatilho, "COD_CAMPANHA" => $cod_campanha));

                $insere = "";
                foreach ($retornoSMS as $key => $cliente) {
                    //if (@$cliente["code"] == "") {
                    // $CHAVE_GERAL = "";
                    // $CHAVE_CLIENTE = "";
                    $msgenvio = @$cliente["body"];
                    ...
                    $sqlControle = "INSERT INTO sms_lista_ret (
												...
												IFNULL((SELECT COD_UNIVEND FROM email_fila WHERE COD_EMPRESA  = 0" . $cod_empresa . " AND COD_CLIENTE = 0" . $cod_cliente . " ORDER BY 1 DESC LIMIT 1),0),
												'0" . $cod_cliente . "',
                    ...
                    //fnLog(array("DESCRICAO"=>"Insere na sms_lista_ret","COD_EMPRESA"=>$cod_empresa,"SQL"=>$sqlControle,"COD_GATILHO"=>$cod_gatilho,"TIP_GATILHO"=>$tip_gatilho, "COD_CAMPANHA" => $cod_campanha));
                    mysqli_query($contemporaria, $sqlControle);
                    if (mysqli_error($contemporaria) != "") {
                        fnLog(array("DESCRICAO" => "Erro SQL ao inserir na sms_lista_ret!", "COD_EMPRESA" => $cod_empresa, "SQL" => $sqlControle, "ERRO" => mysqli_error($contemporaria), "COD_GATILHO" => $cod_gatilho, "TIP_GATILHO" => $tip_gatilho, "COD_CAMPANHA" => $cod_campanha, "LAYOUT" => "text-danger"));
                        // continue;
                    }
                    $qt_envio_real++;
                    //}
                ...
                if ($insere != "") {
                    fnLog(array("DESCRICAO" => "Insere na sms_lista_ret", "COD_EMPRESA" => $cod_empresa, "SQL" => $insere, "COD_GATILHO" => $cod_gatilho, "TIP_GATILHO" => $tip_gatilho, "COD_CAMPANHA" => $cod_campanha));
                }
                ...
                fnLog(array("DESCRICAO" => "<b>Não Enviado</b>", "COD_EMPRESA" => $cod_empresa, "COD_GATILHO" => $cod_gatilho, "TIP_GATILHO" => $tip_gatilho, "ERRO" => json_encode($retornoSMS), "JSON" => json_encode($retornoSMS), "COD_CAMPANHA" => $cod_campanha, "LAYOUT" => "text-danger"));
            }
        }
 ...
 */
        ...
        if ($qt_envio_real > 0) {
            ...
            $rs = mysqli_query($contemporaria, $sql);
            $linha = mysqli_fetch_assoc($rs);
            fnLog(array("DESCRICAO" => "Checa se já tem lote na tabela sms_lote", "JSON" => json_encode($linha), "COD_EMPRESA" => $cod_empresa, "SQL" => $sql, "ERRO" => mysqli_error($contemporaria), "COD_GATILHO" => $cod_gatilho, "TIP_GATILHO" => $tip_gatilho, "COD_CAMPANHA" => $cod_campanha));
            if (@$linha["QTD"] <= 0) {
                ...
                $sqlControle = "INSERT INTO SMS_LOTE(
					...
					'1')";
                mysqli_query($contemporaria, $sqlControle);
                fnLog(array("DESCRICAO" => "Não tem lote. Inserindo na tabela sms_lote", "JSON" => json_encode($retornoSMS), "COD_EMPRESA" => $cod_empresa, "SQL" => $sqlControle, "ERRO" => mysqli_error($contemporaria), "COD_GATILHO" => $cod_gatilho, "TIP_GATILHO" => $tip_gatilho, "COD_CAMPANHA" => $cod_campanha));
                if (mysqli_error($contemporaria) != "") {
                    fnLog(array("DESCRICAO" => "Erro SQL ao inserir na sms_lote", "COD_EMPRESA" => $cod_empresa, "SQL" => $sql, "ERRO" => mysqli_error($contemporaria), "COD_GATILHO" => $cod_gatilho, "TIP_GATILHO" => $tip_gatilho, "COD_CAMPANHA" => $cod_campanha, "LAYOUT" => "text-danger"));
                    // continue;
                }
            } else {
                fnLog(array("DESCRICAO" => "Já tem lote na tabela sms_lote", "COD_EMPRESA" => $cod_empresa, "COD_GATILHO" => $cod_gatilho, "TIP_GATILHO" => $tip_gatilho, "COD_CAMPANHA" => $cod_campanha));
            }
        } else {
