...
while ($rsempresa = mysqli_fetch_assoc($rwempresa)) {
  ...
  $semchave = "UPDATE sms_lista_push SET BOUNCE=1 WHERE  idContatosMailing in (0) AND cod_empresa=" . $rsempresa['COD_EMPRESA'] . " AND CHAVE_GERAL=''";
  ...
  while ($rscontadore = mysqli_fetch_assoc($rwcontadores)) {
    ...
    if ($rwcontadores->num_rows > '0') {
      ...
      $lote = "SELECT * FROM push_lote WHERE LOG_TESTE='" . $rscontadore['LOG_TESTE'] . "' and COD_EMPRESA='" . $rsempresa['COD_EMPRESA'] . "' AND COD_CAMPANHA='" . $rscontadore['COD_CAMPANHA'] . "' AND COD_DISPARO_EXT='" . $rscontadore['ID_DISPARO'] . "'";
      //   echo $lote.'<br>';
      ...
      while ($row = mysqli_fetch_assoc($rwlote)) {
        ...
        if ($rwlote->num_rows > '0') {
          $uplote = "UPDATE push_lote SET QTD_LISTA='" . $total . "'  WHERE LOG_TESTE='$rscontadore[LOG_TESTE]' and COD_EMPRESA='" . $rsempresa['COD_EMPRESA'] . "' AND COD_CAMPANHA='" . $rscontadore['COD_CAMPANHA'] . "' AND COD_DISPARO_EXT='" . $rscontadore['ID_DISPARO'] . "'";
          //  echo $uplote.'<br>';
      ...
      $entregasms = "SELECT * FROM controle_entrega_push WHERE log_teste='$rscontadore[LOG_TESTE]' and COD_EMPRESA='" . $rsempresa['COD_EMPRESA'] . "' AND COD_CAMPANHA='" . $rscontadore['COD_CAMPANHA'] . "' AND COD_DISPARO='" . $rscontadore['ID_DISPARO'] . "'";
      $rwentregasms = mysqli_query($contemporaria, $entregasms);

      //   echo $entregasms.'<br>';
      if ($rwentregasms->num_rows > '0') {
        $updateentrega = "UPDATE controle_entrega_push 
        ...
        $entregue = "INSERT INTO controle_entrega_push (cod_empresa, 
                                                                        cod_campanha_ext,                                                                                
                                                                        cod_campanha, 
                                                                        dat_cadastr, 
                                                                        cod_disparo, 
        ...
        if (!$insertsms) {
          $file = './aquivosX/push_error_insert' . date('YmdHis') . '.txt';
          //  file_put_contents($file, $entregue);
        } else {
          $file = './aquivosX/push_OK_insert' . date('YmdHis') . '.txt';
          //   file_put_contents($file, $entregue);
        }
        //echo $entregue.'<br>';
      }
