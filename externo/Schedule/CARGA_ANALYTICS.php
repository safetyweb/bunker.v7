...
if ($_GET['id'] == '1') {
    ...
    //$dateexec = date('Y-m', strtotime('-1 month', strtotime('2024-05')));
    // $dateexec = date('Y-m', strtotime('-1 month', strtotime("2025-" . $month)));
...
if ($horamenor1 <= $execatual && $horamaior1 >= $execatual) {
    ...
    $sqlEmpresa = "SELECT e.COD_EMPRESA, e.NOM_FANTASI from empresas e
                        INNER JOIN tab_database t ON t.cod_empresa=e.COD_EMPRESA
                    where e.LOG_ATIVO='S' and e.cod_empresa NOT IN (136,514) AND e.LOG_INTEGRADORA = 'N' $codempresa";
        ...
        if ($rsparamentros['COD_FREQUENCIA'] != '') {
            ...
            if (!$execsql) {
                ...
                if ($rsexecsql['RETORNO'] == 'N') {
                    ...
                    sleep(5);
                    $sqlidsesion = "SELECT MAX(ID_SESSION) V_ID_SESSION FROM VENDABASETMP WHERE COD_EMPRESA =" . $rsempresas['COD_EMPRESA'];
                    ...
                    if (!$execCLIEXPIRAR) {
                        ...
                        echo 'Nome empresa: ' . $rsempresas['NOM_FANTASI'] . '<br>';
                        echo 'Executado na empresa:' . $rsempresas['COD_EMPRESA'] . ' </br>';
                        echo 'EXECUTOU SegundaFase' . $sqlCLIEXPIRAR . '<br>';
