       ...
       // echo '</pre>';
        ...
        if($dadositem['Status']=='sttValid')
        {
            if (array_key_exists("0", $dadositem['PartitionDiscount'])) {
                ...
                {
					//alterado aqui fazendo o divisor parapegar o valor unitario do desconto
					//testar na emy/loe cosmesticos
					if($dadosdesc['DiscountAmount'] >= 2)
					{
						$valorliquidoitem=$dadosdesc['Price']/$dadosdesc['DiscountAmount'];
					}else{
						//alterado para pegar o desonto e dividir pela quantidade quando o valor for maior que o unitario do item. 
						 if($dadosdesc['Price'] > $dadositem['UnitPrice']) 
						 {
							 $valorliquidoitem= $dadosdesc['Price'] / $dadositem['Quantity'];
						 }else{							 
