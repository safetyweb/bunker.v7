...
while ($rsempresa = mysqli_fetch_assoc($rwempresa)){
    ...
    echo 'Empresa :'. $rsempresa[COD_EMPRESA].'<br>';    
        ...
        $sqllog="SELECT LOG_JSON,ID_LOG FROM log_nuxux WHERE  cod_empresa=$rsempresa[COD_EMPRESA] AND  DATE(dat_cadastr)>=date(DATE_SUB(NOW(), INTERVAL  6  DAY)) AND TIP_LOG=3 and LOG_PROCESSADO='N' ";
