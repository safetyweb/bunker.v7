	...
	<script>
		$(document).ready(function() {
			$("#fileinput").fileinput({
				...
				uploadUrl: "ajxUploadDocumentos.php?tipo=<?=$tp_anexo?>&ido="+$("#COD_OBJETOANEXO").val()+"&tpc=<?=fnEncode($tp_cont)?>&cod_empresa=<?=$cod_empresa?>&cod_cliente=<?=$cod_cliente?>&num_contador=<?=$num_contador?>",
				uploadAsync: true,
...
<script type="text/javascript">
	...
	function refreshUpload(limpa){
		...
		$.ajax({
			method: 'POST',
			url: 'ajxAttUploadDocumentos.php',
			data: {COD_BUSCA:$('#<?=$cod_tpanexo?>').val(), TP_ANEXO:'<?=$tp_anexo?>', COD_EMPRESA:'<?=$cod_empresa?>', COD_CLIENTE:'<?=$cod_cliente?>', page: page, NUM_CONTADOR:<?=$num_contador?> },
			beforeSend:function(){
			...
			success:function(data){
				...
				$("#fileinput").fileinput("destroy").fileinput({
					...
					var uploadedFileName = file[index].name; // Obtém o nome do arquivo que foi carregado
					$("#DES_DOC").val(uploadedFileName);
					// refreshUpload();
				});
