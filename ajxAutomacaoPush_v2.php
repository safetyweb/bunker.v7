<?php 
	...
	switch($opcao){
		...
		case 'exc':
			...
			$sql = "DELETE FROM TEMPLATE_AUTOMACAO_PUSH WHERE COD_EMPRESA = $cod_empresa AND COD_TEMPLATE = $cod_template";
			// fnEscreve($sql);
			mysqli_query(connTemp($cod_empresa,''),$sql);

			switch ($tipo) {
				case "msg":
					$sql = "DELETE FROM MENSAGEM_PUSH WHERE COD_EMPRESA = $cod_empresa AND COD_TEMPLATE_BLOCO = $cod_template";
					// fnEscreve($sql);
					mysqli_query(connTemp($cod_empresa,''),$sql);
				break;
				case "wait":
					$sql = "DELETE FROM AGUARDO_PUSH WHERE COD_EMPRESA = $cod_empresa AND COD_TEMPLATE = $cod_template";
					// fnEscreve($sql);
					mysqli_query(connTemp($cod_empresa,''),$sql);
				break;
			}
		...
		default:
			...
			$sql = "INSERT INTO TEMPLATE_AUTOMACAO_PUSH(
								COD_EMPRESA,
								COD_CAMPANHA,
								COD_BLTEMPL
								) VALUES(
			...
			$sql = "SELECT * FROM TEMPLATE_AUTOMACAO_PUSH 
					WHERE COD_TEMPLATE = (SELECT MAX(COD_TEMPLATE) FROM TEMPLATE_AUTOMACAO_PUSH 
