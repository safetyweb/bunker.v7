...
$sqlGraph = "SELECT SUM(CEM.QTD_DISPARADOS) AS QTD_DISPARADOS, 
					...
					LEFT JOIN TEMPLATE_SMS TE ON TE.COD_EXT_TEMPLATE = CEM.ID_TEMPLETE AND TE.LOG_ATIVO = 'S'
...
if(isset($_COOKIE['TEMPO_REFRESH_SMS']) && $_COOKIE['TEMPO_REFRESH_SMS'] == true){


    $mostraRefresh = 'false';
    $cookie = json_decode( $_COOKIE["TEMPO_REFRESH_SMS"] );
...
<div class="row portlet">				
	...
	<script type="text/javascript">
		...
		function atualizaDisparos() {
			$.ajax({
				type: "POST",
				url: "ajxRefreshResultadosSms_V2.php",
				data: { COD_EMPRESA:"<?=fnEncode($cod_empresa)?>", COD_CAMPANHA:"<?=fnEncode($cod_campanha)?>"},
				beforeSend:function(){
					$('#blocker').show();
				},
