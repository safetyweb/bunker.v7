...
if($key_des_token == "") $key_des_token = fnLimpaCampo(fnLimpaDoc($_POST['DES_TOKEN']));

if($isApp){

	$k_num_cartao = fnLimpaCampo(fnDecode($_REQUEST['KEY_NUM_CARTAO']));
	$k_num_celular = fnLimpaCampo(fnDecode($_REQUEST['KEY_NUM_CELULAR']));
	$k_cod_externo = fnLimpaCampo(fnDecode($_REQUEST['KEY_COD_EXTERNO']));
	$k_num_cgcecpf = fnLimpaDoc(fnLimpaCampo(fnDecode($_REQUEST['KEY_NUM_CGCECPF'])));
	$k_dat_nascime = fnLimpaCampo(fnDecode($_REQUEST['KEY_DAT_NASCIME']));
	$k_des_emailus = fnLimpaCampo(fnDecode($_REQUEST['KEY_DES_EMAILUS']));

	$cad_nom_cliente = fnLimpaCampo(fnDecode($_REQUEST['CAD_NOM_CLIENTE']));
	$cad_num_cgcecpf = fnLimpaCampo(fnDecode($_REQUEST['CAD_NUM_CGCECPF']));
	$cad_cod_sexopes = fnLimpaCampo(fnDecode($_REQUEST['CAD_COD_SEXOPES']));
	$cad_num_cartao = fnLimpaCampo(fnDecode($_REQUEST['CAD_NUM_CARTAO']));
	$cad_des_emailus = fnLimpaCampoEmail(fnDecode($_REQUEST['CAD_DES_EMAILUS']));
	$cad_des_enderec = fnLimpaCampo(fnDecode($_REQUEST['CAD_DES_ENDEREC']));
	$cad_num_enderec = fnLimpaCampo(fnDecode($_REQUEST['CAD_NUM_ENDEREC']));
	$cad_des_bairroc = fnLimpaCampo(fnDecode($_REQUEST['CAD_DES_BAIRROC']));
	$cad_des_complem = fnLimpaCampo(fnDecode($_REQUEST['CAD_DES_COMPLEM']));
	$cad_des_cidadec = fnLimpaCampo(fnDecode($_REQUEST['CAD_DES_CIDADEC']));
	$cad_cod_estadof = fnLimpaCampo(fnDecode($_REQUEST['CAD_COD_ESTADOF']));
	$cad_num_cepozof = fnLimpaCampo(fnDecode($_REQUEST['CAD_NUM_CEPOZOF']));
	$cad_dat_nascime = fnLimpaCampo(fnDecode($_REQUEST['CAD_DAT_NASCIME']));
	$cad_num_celular = fnLimpaCampo(fnDecode($_REQUEST['CAD_NUM_CELULAR']));
	$cad_cod_profiss = fnLimpaCampo(fnDecode($_REQUEST['CAD_COD_PROFISS']));
	$cad_cod_atendente = fnLimpaCampo(fnDecode($_REQUEST['CAD_COD_ATENDENTE']));
	...
	$k_des_emailus = fnLimpaCampoEmail($_REQUEST['KEY_DES_EMAILUS']);
	...
	$cad_des_emailus = fnLimpaCampoEmail($_REQUEST['CAD_DES_EMAILUS']);
	...
	switch ($qrCampos[DES_CAMPOOBG]) {
		...
		case 'DES_EMAILUS':
			...
			if($_POST[$qrCampos[DES_CAMPOOBG]] != ""){
				...
				if(strstr($_POST[$qrCampos[DES_CAMPOOBG]], '*')){
					$email = fnLimpaCampoEmail(fnDecode($_POST['HID_'.$qrCampos[DES_CAMPOOBG]]));
				}else{
					$email = fnLimpaCampoEmail($_POST[$qrCampos[DES_CAMPOOBG]]);
				}
		...
		case 'NUM_CELULAR':
			...
			if($_POST[$qrCampos[DES_CAMPOOBG]] != ""){
				...
				if(strstr($_POST[$qrCampos[DES_CAMPOOBG]], '*')){
					$telefone = fnLimpaCampo(fnLimpaDoc(fnDecode($_POST['HID_'.$qrCampos[DES_CAMPOOBG]])));
				}else{
		...
		case 'DAT_NASCIME':
			...
			if($_POST[$qrCampos[DES_CAMPOOBG]] != ""){
				...
				if(strstr($_POST[$qrCampos[DES_CAMPOOBG]], '*')){
					$dt_nascimento = fnLimpaCampo(fnDecode($_POST['HID_'.$qrCampos[DES_CAMPOOBG]]));
				}else{
					$dt_nascimento = fnLimpaCampo($_POST[$qrCampos[DES_CAMPOOBG]]);
				}
...
if($_POST[DES_SENHAUS] != ""){

	$senha = trim($_POST[DES_SENHAUS]);

	if($isApp && $_POST[DES_SENHAUS_ENC] != ""){
		$senha = trim(fnDecode($_POST[DES_SENHAUS]));
	}

}


if($_POST[COD_UNIVEND] != "" && $_POST[COD_UNIVEND] != 0){

	$arrayCampos[2] = $_POST[COD_UNIVEND];

}

if($_POST[COD_ATENDENTE] != "" && $_POST[COD_ATENDENTE] != 0){

	$codatendente = $_POST[COD_ATENDENTE];

}

if($_POST[COD_INDICAD] != "" && $_POST[COD_INDICAD] != 0){

	$codIndicad = $_POST[COD_INDICAD];
...
if($tip_campanha == 23){
	$adesao = "QR_#".$chaveCampanha;
}

if($isApp && $cod_empresa == 219){
	$key_des_token = "4PPL3";
}
// exit();
//dados atualiza cadastro
$dadosatualiza=Array('nome'=>fnAcentos($nome),
                        'sexo'=>$sexo,
                        'email'=>$email,
                        'telefone'=>$telefone,
                        'cpf'=>$cpf,
...
if($cartao == '34997927730' || $cartao == '15981146246' || $cartao == '39648555885' || $cartao == '01734200014' || $cartao == '47930182894'){
	// echo "<pre>";
...
if($atualiza == "Registro inserido!" || $atualiza == "Cadastro Atualizado !"){
	...
	if($atualiza == "Registro inserido!"){
		$atualiza = "realizado com sucesso!";
	}else{
		$atualiza = "atualizado com sucesso.";
	}
	...
	if ($cod_tpfiltro != ""){
		...
		$attUnivCli = "INSERT INTO CLIENTE_FILTROS(
										COD_EMPRESA,
										COD_TPFILTRO,
										COD_FILTRO,
										COD_CLIENTE,
	...
	if($log_lgpd == 'S' && $adesao == 'CT'){
		mysqli_query(connTemp($cod_empresa,''),$sqlUpdCli);
	}

	if($atualiza == "realizado com sucesso!" || $ativacao == 1){
