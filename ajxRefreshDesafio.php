...
if($cod_desafio != 0){
	...
	$sqlExc = "DELETE FROM DESAFIO WHERE COD_EMPRESA = $cod_empresa AND COD_DESAFIO = $cod_desafio; ";
	$sqlExc .= "DELETE FROM DESAFIO_CONTROLE WHERE COD_EMPRESA = $cod_empresa AND COD_DESAFIO = $cod_desafio; ";
...
$sql = "SELECT DESAFIO.*,
		...
		FROM DESAFIO WHERE DESAFIO.COD_EMPRESA = $cod_empresa order by DAT_INI desc ";
		//fnEscreve($sql);
