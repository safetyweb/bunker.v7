...
$TIP_PAGTO = ($_REQUEST["TIP_PAGTO"] == 1 ? "CARTAO" : ($_REQUEST["TIP_PAGTO"] == 3 ? "PIX" : "BOLETO"));
$_REQUEST["VAL_TOTAL_DESC"] = str_replace(",", ".", str_replace(".", "", "0" . @$_REQUEST["VAL_TOTAL_DESC"]));
$VAL_TOTAL = (fnLimpaCampo($_REQUEST["VAL_TOTAL"]) - fnLimpaCampo($_REQUEST["VAL_TOTAL_DESC"]));
...
switch (@$_REQUEST['acao']) {
  case "cad_pedido":
    ...
    if (fnLimpaCampo($_REQUEST["VAL_TOTAL_DESC"]) <> "") {
      $_REQUEST["QTD_EMAIL_TOTAL"] = round($_REQUEST["QTD_EMAIL_TOTAL"] * ($VAL_TOTAL / fnLimpaCampo($_REQUEST["VAL_TOTAL"])), 2);
      $_REQUEST["QTD_WPP_TOTAL"] = round($_REQUEST["QTD_WPP_TOTAL"] * ($VAL_TOTAL / fnLimpaCampo($_REQUEST["VAL_TOTAL"])), 2);
      $_REQUEST["QTD_SMS_TOTAL"] = round($_REQUEST["QTD_SMS_TOTAL"] * ($VAL_TOTAL / fnLimpaCampo($_REQUEST["VAL_TOTAL"])), 2);
      $_REQUEST["VAL_TOTAL"] = $VAL_TOTAL;
    }
    ...
    if ($_REQUEST["tpOperacao"] != "CREDITO") {
      ...
      $params["paymentMethod"] = ($TIP_PAGTO == "CARTAO" ? "creditCard" : ($TIP_PAGTO == "PIX" ? "pix" : "boleto")); # ou BOLETO ou ONLINE_DEBIT
      $params["currency"] = "BRL";
      ...
      for ($i = 0; $i < count($arrProduto); $i++) {
        ...
        $params["itemAmount{$tmpId}"] = number_format($arrProduto[$i]["VAL_UNITARIO"], 2, ".", "");
        $params["itemQuantity{$tmpId}"] = $arrProduto[$i]["QTD_PRODUTO"];
      }
      ...
      if (strlen($NUM_DOCUMENTO) <= 11) {
        ...
        $params["senderCNPJ"] = "$NUM_DOCUMENTO";
      }
      ...
      $params["installmentValue"] = number_format("$VAL_TOTAL", 2, ".", ""); # Valor da parcela

      //$params["instructions"] = "Marka Sistema de Fideliza&ccedil;&atilde;o";
      //$params["description"] = "Produto";
    ...
    if ($tpPag != "") {
      ...
      $sqlIns = " INSERT INTO PEDIDO_MARKA (
                        COD_ORCAMENTO, COD_PRODUTO, QTD_PRODUTO, QTD_SALDO_ATUAL, VAL_UNITARIO, COD_EMPRESA, COD_UNIVEND
                        , ID_SESSION_PAGSEGURO, ID_HASH_PAGSEGURO, PAG_CONFIRMACAO,PARAM_ENVIO
						, RETORNO_JSON_PAGSEGURO,QTD_DIAS) 
                      SELECT 
                        COD_ORCAMENTO, COD_PRODUTO, QTD_PRODUTO, QTD_PRODUTO, VAL_UNITARIO, COD_EMPRESA, '$COD_UNIVEND' AS COD_UNIVEND
                        , '$cdPag' AS ID_SESSION_PAGSEGURO, '$cardHash' AS ID_HASH_PAGSEGURO, '$tpPag' AS PAG_CONFIRMACAO , '" . json_encode($params) . "' AS PARAM_ENVIO
						, '" . json_encode($dadosJSon) . "',QTD_DIASVALID
                          FROM TMP_PEDIDO_MARKA WHERE COD_ORCAMENTO = '$cod_orcamento'";
      mysqli_query($connAdm->connAdm(), $sqlIns) or die(mysqli_error() . $sqlIns);
    }
...
function cadastraProdutos($connAdm, $cod_empresa, $identificador)
{
  $arr = array();
  $qtd_produto = fnLimpaCampoZero($_REQUEST["QTD_{$identificador}"]);
  if ($qtd_produto > 0) {
    $total = fnLimpaCampoZero($_REQUEST["QTD_{$identificador}_TOTAL"]);
    $val_uni = round($total / $qtd_produto, 2);
    $cod_canalcom = fnLimpaCampoZero($_REQUEST["QTD_{$identificador}_COD_CANALCOM"]);
    $cod_comfaixa = fnLimpaCampoZero($_REQUEST["QTD_{$identificador}_COD_COMFAIXA"]);
    $cod_produto = retornaCodProduto($connAdm, $cod_canalcom, $cod_comfaixa, $qtd_produto, $cod_empresa, 1);
    $nom_produto = retornaNomProduto($connAdm, $cod_canalcom, $cod_comfaixa);
    $valid_produto = retornaValidProduto($connAdm, $cod_canalcom, $cod_comfaixa,  $qtd_produto);
    $arr["COD_PRODUTO"] = $cod_produto;
    $arr["NOM_PRODUTO"] = $nom_produto . " " . $identificador;
    $arr["QTD_PRODUTO"] = $qtd_produto;
    $arr["VAL_UNITARIO"] = $val_uni;
    $arr["VAL_TOTALPRO"] = $total;
    $arr["QTD_DIASVALID"] = $valid_produto;
  }
  return $arr;
}
...
function retornaNomProduto($connAdm, $cod_canalcom, $cod_comfaixa)
{
  $sql = "SELECT CF.NOM_FAIXA 
          FROM COMUNICACAO_PRECO CP
          INNER JOIN COMUNICACAO_FAIXAS CF ON CF.COD_COMFAIXA = CP.COD_COMFAIXA
          WHERE CP.COD_CANALCOM = 0{$cod_canalcom} AND CP.COD_COMFAIXA = 0{$cod_comfaixa} ";
  $result = mysqli_query($connAdm->connAdm(), $sql) or die(mysqli_error());
  $row = mysqli_fetch_assoc($result);
  return @$row["NOM_FAIXA"];
}

function retornaValidProduto($connAdm, $cod_canalcom, $cod_comfaixa, $qtd_produto)
{
  global $cod_empresa;
  $sql = "SELECT CP.QTD_DIASVALID
          FROM COMUNICACAO_PRECO CP
          INNER JOIN COMUNICACAO_FAIXAS CF ON CF.COD_COMFAIXA = CP.COD_COMFAIXA
          WHERE CP.COD_CANALCOM = 0{$cod_canalcom} AND CP.COD_COMFAIXA = 0{$cod_comfaixa}
          AND CP.COD_SISTEMA IN (SELECT COD_SISTEMAS FROM empresas E WHERE COD_EMPRESA=$cod_empresa) ";
  $result = mysqli_query($connAdm->connAdm(), $sql) or die(mysqli_error());
  $row = mysqli_fetch_assoc($result);
  return @$row["QTD_DIASVALID"];
}
