  ...
  OrgChart.prototype = {
    ...
    loopChart: function ($chart) {
      ...
      var subObj = {
						...
						'cod_usuario': ($node.attr("cod_usuario") != undefined ? $node.attr("cod_usuario") : ""),
						'nom_usuario': ($node.attr("nom_usuario") != undefined ? $node.attr("nom_usuario") : ""),
						'des_objetivo': ($node.attr("des_objetivo") != undefined ? $node.attr("des_objetivo") : ""),
						'des_funcao': ($node.attr("des_funcao") != undefined ? $node.attr("des_funcao") : ""),
    ...
    createNode: function (data) {
	  ...
	  /*
	  if (data[opts.nodeTitle] != undefined){
	  ...
	  */
	  data.nom_usuario = (data.nom_usuario == undefined?"":data.nom_usuario);
	  data.cod_usuario = (data.nom_usuario == undefined?"":data.cod_usuario);
	  data.des_objetivo = (data.des_objetivo == undefined?"":data.des_objetivo);
	  data.des_funcao = (data.des_funcao == undefined?"":data.des_funcao);
      var $nodeDiv = $('<div class="oc" '
						+ (opts.draggable ? ' draggable="true"' : '')
						+ (data[opts.nodeId] ? ' id="' + data[opts.nodeId] + '"' : '')
						+ (data.parentId ? ' data-parent="' + data.parentId + '"' : '')
						+ (data.cod_usuario ? ' cod_usuario="' + data.cod_usuario + '"' : '')
						+ (data.nom_usuario ? ' nom_usuario="' + data.nom_usuario + '"' : '')
						+ (data.des_objetivo ? ' des_objetivo="' + data.des_objetivo + '"' : '')
						+ (data.des_funcao ? ' des_funcao="' + data.des_funcao + '"' : '')
      ...
      if (opts.nodeTemplate) {
        ...
        $nodeDiv
		  .append('<div class="title" title=""><div style="text-align:center;width:110px;overflow: hidden;float:left;" class="NOM_USUARIO">' + data.nom_usuario + '</div>'
					+'<a href="javascript:" style="margin-top:-2px;float:right;color:#555;" class="btn-edit text-delete dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fal fa-pen f10"></i></a>'
					+'<div class="dropdown-menu" style="width:300px;">'
						+'<fieldset>'
							+'<div class="form-group">'
								+'<label for="inputName" class="control-label">Nome</label>'
								+'<div class="input-group">'
									+'<input type="text" class="form-control input-sm" readonly name="NOM_USUARIO" value="'+data.nom_usuario+'">'
									+'<input type="hidden" name="COD_USUARIO" value="'+data.cod_usuario+'">'
									+'<a href="javascript:" class="addBox" data-url="action.php?mod='+modUsuarios+'&id='+cod_empresa+'&pop=true" data-title="Busca Usu&aacute;rios"><span class="input-group-addon">'
										+'<i class="fas fa-search"></i>'
									+'</span></a>'
								+'</div>'
							+'</div>'
							+'<div class="form-group">'
								+'<label for="inputName" class="control-label">Fun&ccedil;&atilde;o</label>'
								+'<input type="text" class="form-control input-sm" onChange="atualizaOrg()" name="DES_FUNCAO" value="'+data.des_funcao+'">'
							+'</div>'
							+'<div class="form-group">'
								+'<label for="inputName" class="control-label">Objetivo</label>'
								+'<input type="text" class="form-control input-sm" onChange="atualizaOrg()" name="DES_OBJETIVO" value="'+data.des_objetivo+'">'
							+'</div>'
						+'</fieldset>'
					+'</div>'
				+'</div>')
          .append('<div class="content" title=""><div style="width:100px;overflow: hidden;float:left;">Fun&ccedil;&atilde;o: <b><span class="DES_FUNCAO">' + data.des_funcao + '</span></b></div></div>')
		  .append('<div class="content" title=""><div style="width:100px;overflow: hidden;float:left;">Objetivo: <b><span class="DES_OBJETIVO">' + data.des_objetivo + '</span></b></div></div>')
		  .append(' <small><a href="javascript:" title="" class="btn-add text-success"><i class="fas fa-plus-circle"></i></a></small>')
		  .append(' <small><a href="javascript:" title="" class="btn-delete text-danger"><i class="fas fa-minus-circle"></i></a></small>');
      }
      //
