...
#colorPick{z-index:99999}
</style>
<script>
	var gravaCor = false;
	function corChamado(cod_chamado,cor){
		if (!gravaCor) {return false;}
...
<?php
	...
	if($dat_fim == ""){$ANDdatFim = " ";}else{$ANDdatFim = "AND DATE_FORMAT(SC.DAT_CADASTR, '%Y-%m-%d') <= '$dat_fim' ";}
	...
	if(@$cod_usuario_ordenac == ""){$ANDcod_usuario_ordenac = " ";}else{$ANDcod_usuario_ordenac = "AND IFNULL(SC.COD_USUARIO_ORDENAC, 0) = $cod_usuario_ordenac ";}
	...
	$rs = mysqli_query($connAdmSAC->connAdm(),$sqlCount) or die(mysqli_error());
	...
	$sqlSac = "SELECT SC.COD_CHAMADO, SC.COD_EMPRESA, SC.NOM_CHAMADO, SC.COD_EXTERNO, 
				...
				SC.COD_USURES, SC.LOG_ADM, SP.DES_PLATAFORMA, ST.DES_TPSOLICITACAO, SC.COD_STATUS,
	  ...
	  <?php if ($manutencao){ ?>
		  ...
		  <td class='text-center'>
			<a href="#" class="editable-data" 
				...
				data-pk="<?=$qrSac[COD_CHAMADO]?>"
				data-name="DAT_ENTREGA" style='border:none;'><small><?=$entrega?></small>
...
$(document).ready(function(){
	...
	$(function(){
		$('.editable').editable({ 
			...
			success:function(data){
				var cod_chamado=$(data).siblings(".COD_CHAMADO").val();
				var dat_entrega=$(data).siblings(".DAT_ENTREGA").val();
				if (cod_chamado != undefined){
					if (dat_entrega != undefined){
						$("[data-pk="+cod_chamado+"][data-name='DAT_ENTREGA'] small").html(dat_entrega);
					}
				}
				console.log(data);
			}
